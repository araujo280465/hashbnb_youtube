...
const Place = () => {
  ...
  const handleBooking = async (event) => {
    ...
    if (checkin && checkout && guests) {
      ...
      alert("Preencha todas as informações antes de fazer um areserva !!");
    }
  };
        ...
        <div className="relative grid aspect-square gap-4 overflow-hidden rounded-2xl sm:aspect-[3/2] sm:grid-cols-[2fr_1fr] sm:grid-rows-2">
          {place.photo
            ...
            .map((p, index) => (
              <img
                key={index}
                className={`${index === 0 ? "row-span-2 h-full object-center" : ""} aspect-square w-full cursor-pointer object-cover transition hover:opacity-75 sm:object-center`}
                src={p}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          <form className="order-1 flex flex-col gap-4 self-center justify-self-center rounded-2xl border border-gray-300 px-4 py-3 sm:px-8 sm:py-4 md:order-none">
            ...
            <div className="flex flex-col gap-2 rounded-2xl border border-gray-300 px-4 py-2">
              <p className="font-bold">Número de convidados: </p>
              <input
            ...
            {user ? (
              <button
                className="bg-primary-400 hover:bg-primary-500 w-full cursor-pointer rounded-full border border-gray-300 px-4 py-2 text-center font-bold text-white"
